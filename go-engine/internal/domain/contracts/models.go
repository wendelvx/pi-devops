...
type Boss struct {
	...
	AvatarURL   string `json:"avatar_url"`  // NOVO: Foto do Professor
}
...
type Incident struct {
	...
	Duration    int    `json:"duration"`     // NOVO: Quantos segundos o time tem para resolver
}

// NOVO: Estrutura para calcular quem bateu mais no Boss
...
type GameState struct {
	...
	TeamHP         int         `json:"team_hp"`         // NOVO: Vida da Equipe
	...
	IncidentTimer  int         `json:"incident_timer"`  // NOVO: Cronômetro regressivo
	CurrentBoss    Boss        `json:"current_boss"`    // Dados do professor atual
	MVP            *PlayerStat `json:"mvp"`             // NOVO: Guarda o destaque da rodada
}