...
func GenerateIncident() contracts.Incident {
	incidents := []contracts.Incident{
		// --- SECURITY ---
		{
			...
			Points:              350,
			Duration:            15,
			RequiredResolutions: 5, // 5 acertos coletivos para anular
		},
		{
			ID:                  "sec_sql_inj",
		...
		// --- DEVOPS ---
		{
			...
			Points:              300,
			Duration:            10,
			RequiredResolutions: 10, // RAPID_CLICK pode exigir mais esforço
		},
		{
			ID:                  "dev_pod_eviction",
			Title:               "Pod Eviction",
	...
	fmt.Printf("⚠️ INCIDENTE ATIVO: %s para a classe %s (Duração: %ds, Meta: %d)\n", selected.Title, selected.TargetClass, selected.Duration, selected.RequiredResolutions)
